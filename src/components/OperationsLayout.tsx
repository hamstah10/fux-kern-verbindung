  ...
  return (
    <OperationsTabsContext.Provider value={{ tabs, activeTabId, addTab, removeTab, setActiveTab: setActiveTabId, cycleTabColor }}>
      <div className="min-h-screen flex flex-col bg-background text-foreground">
        ...
        {/* Tabs Bar */}
        {tabs.length > 0 && (
           <div className="relative bg-background/60 border-b border-border/50">
            <div className="relative max-w-7xl mx-auto flex items-center h-10 px-6 gap-0.5 overflow-x-auto">
              {tabs.map(tab => {
                ...
                return (
                  <div
                    key={tab.id}
                    className={`group relative flex items-center gap-1.5 px-3 h-full text-xs cursor-pointer border-b-2 -mb-px transition-all select-none shrink-0 py-2 ${
                      isActive
